...
<article class="<?php echo $post->post_status; ?> post-list-item">
	<div class="container mt-2 mb-5 pb-sm-4">
		<div class="row form-group row align-items-center justify-content-center">
			<label for="staffSearch" class="col-auto col-form-label pr-0">Search</label>
			<div class="col-sm-4">
				<input type="text" id="staffSearch" name="staffSearch" class="form-control form-control-sm">
			</div>
			<div class="col-auto">
				<button id="reset-search" class="btn btn-sm btn-default">Reset</button>
			</div>
		</div>

		<table id="directory-table" class="table table-sm small table-hover table-responsive">
			<thead>
				<tr class="bg-primary">
					<th>Name</th>
					<th>Department</th>
				...
				<?php foreach ($alphabetized as $post) : ?>
					...
					<?php else : ?>
						<tr>
							...
							<td>Staff</td>
							<td><?php if ($job_title = get_field('person_jobtitle', $post->ID)) {
											echo $job_title;
										} ?></td>
							<td><?php if ($phone = get_field('person_phone', $post->ID)) {
											echo $phone;
										} ?></td>
							<td><?php if ($email = get_field('person_email', $post->ID)) {
